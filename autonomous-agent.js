...
class AutonomousHealthcareAgent {
  constructor() {
    ...
    // Initialize MCP clients (async, non-blocking)
    this.initializeMCPClient().catch(error => {
      console.log(chalk.red(`❌ ElevenLabs MCP initialization failed: ${error.message}`));
      this.mcpConnected = false;
    });

    this.initializeRailwayMCPClient().catch(error => {
      console.log(chalk.red(`❌ Railway MCP initialization failed: ${error.message}`));
      this.mcpRailwayConnected = false;
    });
  }
  ...
  /**
   * Initialize Railway MCP Client for deployment management
   * For now, we'll just use direct GraphQL API calls since MCP Railway integration
   * requires additional setup and the CLI is not available in the container
   */
  async initializeRailwayMCPClient() {
    try {
      console.log(chalk.yellow(`🚂 Initializing Railway API integration...`));

      // Check if we have Railway API token
      if (!this.config.railwayToken || this.config.railwayToken.includes('dummy')) {
        throw new Error('Railway API token not configured');
      }

      // For now, we'll mark as not connected and use direct GraphQL
      // In the future, we could implement a proper Railway MCP server
      console.log(chalk.yellow(`🚂 Railway configured for direct GraphQL API calls`));
      
      this.mcpRailwayConnected = false; // Force GraphQL usage for now
    } catch (error) {
      console.log(chalk.red(`❌ Railway MCP Connection failed: ${error.message}`));
      console.log(chalk.yellow(`⚠️ Fallback: Using direct GraphQL API calls for Railway management`));
      this.mcpRailwayConnected = false;
    }
  }
...
${conversationHistory.map(msg => `${msg.role}: ${msg.content}`).join('\n')}`;
    ...
    const tools = [
      ...
      },
      {
        type: "function", 
        function: {
          name: "evaluate_clinic_relevance",
          description: "Evaluate if a clinic is relevant for cosmetic/aesthetic treatments",
          parameters: {
            type: "object",
            properties: {
              clinic_title: {
                type: "string",
                description: "Title of the clinic"
              },
              clinic_url: {
                type: "string", 
                description: "URL of the clinic"
              },
              user_query: {
                type: "string",
            ...
            required: ["clinic_title", "clinic_url", "user_query"]
          }
        }
      }
    ];
  ...
  async handleToolCalls(aiMessage, originalQuery) {
    console.log('🔧 Processing AI tool calls...');
    
    for (const toolCall of aiMessage.tool_calls) {
      const functionName = toolCall.function.name;
      const functionArgs = JSON.parse(toolCall.function.arguments);
      
      console.log(`🛠️ AI wants to use tool: ${functionName}`);
        ...
        switch (functionName) {
          case 'web_search_exa':
            ...
            if (results.length > 0) {
              ...
              return {
                executeWorkflow: false,
                response: `❌ No cosmetic clinics found in ${functionArgs.location} matching your criteria. Try a different location or broader search terms.`
              };
            }
            
          case 'evaluate_clinic_relevance':
            const isRelevant = await this.evaluateClinicWithAI(functionArgs);
            console.log(`🤔 Clinic relevance: ${isRelevant}`);
            return {
              executeWorkflow: false,
              response: `Based on "${functionArgs.clinic_title}", this clinic is ${isRelevant ? 'relevant' : 'not relevant'} for your cosmetic treatment needs.`
            };
          ...
          default:
            ...
            return {
              executeWorkflow: false,
              response: `I tried to use a tool called "${functionName}" but it's not available yet.`
            };
        }
      } catch (error) {
        console.log(`❌ Tool execution error for ${functionName}:`, error);
        return {
          executeWorkflow: false,
          response: `Sorry, I had trouble using the ${functionName} tool. Please try again.`
        };
      }
    }
  }

  async executeEXASearch(params) {
    console.log('🔍 AI agent executing EXA search with params:', params);
    ...
    try {
      ...
      for (const practice of practices) {
        ...
        if (isRelevant) {
          ...
          console.log(`✅ AI approved: ${practice.title}`);
        } else {
  ...
  async evaluateClinicWithAI(params) {
...
Respond with only "RELEVANT" or "NOT_RELEVANT"`;
      ...
      return false; // Default to not relevant if evaluation fails
    }
  }
...
🎉 Healthcare lead generation finished!
  ...
  async executeCustomAutonomousWorkflow(workflowConfig) {
    console.log(chalk.blue('🚀 Starting AI Agent Autonomous Workflow'));
    ...
    let healthcarePractices;
    
    // Check if AI already found clinics (from function calling)
    if (workflowConfig.preFoundClinics && workflowConfig.preFoundClinics.length > 0) {
      console.log(chalk.cyan(`🤖 AI AGENT: Using ${workflowConfig.preFoundClinics.length} pre-found clinics from AI search`));
      healthcarePractices = workflowConfig.preFoundClinics;
    } else {
      // Fallback to traditional search if no pre-found clinics
      const customSearchQuery = workflowConfig.searchQuery || this.generateCustomSearchQuery(workflowConfig);
      console.log(chalk.cyan(`🔍 FALLBACK: Traditional EXA Search for ${leadCount} practices`));
      console.log(chalk.cyan(`🔍 Query: ${customSearchQuery}`));
      
      healthcarePractices = await this.findHealthcarePracticesWithCustomEXA(customSearchQuery, leadCount, workflowConfig);
    }
    
    if (!healthcarePractices || healthcarePractices.length === 0) {
      throw new Error(`No cosmetic clinics found. AI agent search returned no results.`);
    }
    
    console.log(chalk.green(`✅ Processing ${healthcarePractices.length} AI-validated cosmetic clinics`));
  ...
  async railwaySetVariables(projectId, environmentId, serviceId, practiceData) {
    console.log(`   🔧 Setting Railway environment variables via MCP for service: ${serviceId}`);
    ...
    try {
      // Use Railway MCP tools directly instead of CLI
      if (this.mcpRailwayConnected && this.mcpRailwayClient) {
        console.log(`   🎯 Using Railway MCP tools for variable management`);
        
        const railwayVariables = {};
        for (const [key, value] of Object.entries(variables)) {
          railwayVariables[key] = String(value);
        }
        
        const mcpResult = await this.mcpRailwayClient.callTool('variable_bulk_set', {
          projectId: projectId,
        ...
        console.log(`   ✅ All Railway environment variables set successfully via MCP`);
        return mcpResult;
      }
      
      // Fallback to direct GraphQL API calls
      console.log(`   🔗 Using direct GraphQL API for Railway variables`);
  ...
  async railwayCreateDomain(projectId, environmentId, serviceId) {
    console.log(`   🌐 Creating Railway domain via MCP for service: ${serviceId}`);
    console.log(`   🔗 Using projectId: ${projectId}, environmentId: ${environmentId}`);
    
    try {
      // Use Railway MCP tools directly instead of CLI
      if (this.mcpRailwayConnected && this.mcpRailwayClient) {
        console.log(`   🎯 Using Railway MCP tools for domain creation`);
        
        const mcpResult = await this.mcpRailwayClient.callTool('domain_create', {
          environmentId: environmentId,
          serviceId: serviceId
        });
        
        console.log(`   ✅ Railway domain created successfully via MCP`);
        
        return {
          url: `https://${mcpResult.domain}`,
          domain: mcpResult.domain
        };
      }
      
      // Fallback to direct GraphQL API calls
      console.log(`   🔗 Using direct GraphQL API for Railway domain creation`);
      ...
      console.log(`   ❌ Railway domain creation failed:`, error.message);
      throw new Error(`Railway domain creation completely failed: ${error.message}`);
    }
  }
